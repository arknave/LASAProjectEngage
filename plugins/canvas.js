...
jQuery.fn.extend({
  ...
  wrap_script: function(number){
      // wrap the top-level script to prevent leaking into globals
      var script = this.pretty_script();
      var retval =  '(function($){' +
                    'try{' +
                    //'var me = $("#stagesprite"+number);' +
					'$("#execute_scripts").bind("click", function(event) {' +
                    script +
					'});' +
...
function setup(){
	...
	function compile_scripts(event){
		...
		//$("#execute_scripts").click(blocks.wrap_script());
	}
    ...
    var menus = {
        control: menu('Control', [
            {
                label: 'when program [number: ] runs',
                trigger: true,
                slot: false,
                containers: 1,
                script: 'function _start{{1}}(){var me = $("#stagesprite{{1}}"); console.log($("#stagesprite{{1}} > canvas")[0]); var mectx = $("#stagesprite{{1}} > canvas").get(0).getContext("2d"); [[1]]}_start{{1}}();',
        ...
        img: menu('Image Manipulation', [
			...
			{
				label: 'get pixel at coordinates [number: 0] [number: 0]',
				script: '(function() { var data = mectx.getImageData({{1}}, {{2}}, 1, 1); return {r: data[0], g: data[1], b: data[2]};})()',
				type: 'pixel'
			},
            {
                label: 'sprite width',
        ...
        neo: menu('Neo', [
            {
                ...
                script: "me.animate({top: '+={{1}}'});",
                help: 'Moves the sprite down a certain number of pixels'
            },
			{
                label: 'move to point [point]',
                script: 'me.css("left:" {{1}}.x,"top:" {{1}}.y);',
				
                help: 'move to...'
            },
            {
                label: 'Glide left [number: ]',
                script: 'me.animate({left: "-={{1}}"});',
                help: 'Move left a specified number of pixels'
            },
        ])
    };
