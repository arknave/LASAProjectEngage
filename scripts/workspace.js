(function($) {
    ...
    function populate_and_show_restore_dialog() {
        ...
        for ( idx = 0; idx < localStorage.length; idx++) {
            ...
            script_li.data('scripts', script_obj.scripts);
            // avoid re-parsing later
    ...
    function populate_demos_dialog(demos) {
        ...
        $.each(demos, function() {
            ...
            script_li.data('scripts', this.scripts);
            // avoid re-parsing later
            list.append(script_li);
        });
    }
    ...
    window.populate_demos_dialog = populate_demos_dialog;
    ...
    window.restore_demo_by_name = restore_demo_by_name;
    // expose as public so we can link to a demo.
    ...
    $('#restore_dialog').delegate('.restore', 'click', restore_named_scripts).delegate('.show_description', 'click', toggle_description).delegate('.delete', 'click', delete_named_scripts);

    $('#demos_dialog').delegate('.load', 'click', restore_demo_scripts).delegate('.show_description', 'click', toggle_description);
    $('#demos_dialog .cancel').click(function() {
        $('#demos_dialog').bPopup().close();
    });
    $('.demo_scripts').click(function() {
        $('#demos_dialog').bPopup();
    });
